...
#' @details
#' The model is
#' \deqn{y_t = B_y(L) y_t + B_x x_t + \epsilon_t},
#' where \eqn{y_t} is an `nv` by 1 vector, \eqn{B_x(L)} is an
...
#' some "own lag" coefficients.  `OwnLagMeans`
...
#'
#' The usual case, with a constant term and no other exogenous variable, arises
#' with `nx=1`, `xbar=1`, `xsig=0`.
...
#' @param sig  Modal prior std deviations of residuals.  This vector
    ...
    { ## single-coefficient prior dummy obs.
        ...
        ydum <- array(0,dim=c(lags+1,nv,lags,nv),
                      dimnames=list(obsno=1:(lags+1),
                                    ...
                                    lag=1:lags,
                                    rhsy=1:nv))
        ## lines below appear redundant and mistaken, given
        ## xdum setting above.
        ##        if (nx > 0) {
