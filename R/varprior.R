...
#'
#' The model is
#' \deqn{y_t = B(L) y_t + \epsilon_t},
#' where \eqn{y_t} is an `nv` by 1 vector, \eqn{B(L)} is an
#' `nv` by `nv` matrix polynomial in strictly positive powers of the
#' lag operator \eqn{L}, and
#' \eqn{\epsilon_t \sim N(0, \Sigma){\epsilon_t ~ N(0, \Sigma)} conditional on all
#' \eqn{y_s, s < t}.
#' 
