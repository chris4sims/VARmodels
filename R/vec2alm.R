...
#' @md
#' 
vec2alm <- function(x, nv, vnames=NULL) {
    ...
    ## ----------for Dirichlet prior on lmd where rows are mean 1
    ## lmd <- abind(lmd, nSig - apply(lmd, 1:2, sum), along=3)
    ##----------------------------------------------------
    lmd <- abind(lmd, exp(-apply(log(lmd), 1:2, sum)), along=3)
    A <- aperm(A, c(2, 3, 1))
    ...
    if(!is.null(vnames)) {
      dimnames(A)[[2]] <- vnamesDec
      dimnames(lmd)[[1]] <- vnames
    }
