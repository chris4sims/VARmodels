varDD <- function(ydata=NA, xdata=NA, breaks=NULL, lambda=5, mu=2, ic=NULL, Ay, Ax, ywt=NULL, xwt=NULL, rhs=NULL, ywti=NULL, rhsi=NULL) {
  lags <- dim(Ay)[3] - 1
  realsmall <- 1e-15
  T<-dim(ydata)[1];
  nvar<-dim(ydata)[2];
...
### matlab code uses the fact that in matlab a:b is null if b<a, which is not true for R.
  ...
  llh2 <- ADobsLH( Ay, Ax, ywt, xwt, rhs, ywti, rhsi )
